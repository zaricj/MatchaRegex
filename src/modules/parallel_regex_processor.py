...
class ParallelRegexProcessor(QObject):
    """Coordinator that manages parallel file processing"""
    ...
    def run(self):
        ...
        try:
            ...
            self.signals.program_output_text.emit(
                f"Starting parallel processing of {self.total_files} files")
            ...
            self.signals.message_critical.emit(
                "Search Error", f"Coordinator error: {str(e)}")
            self.signals.finished.emit(pd.DataFrame())
    ...
    def _on_file_processed(self, file_index: int):
        ...
        with QMutexLocker(self.files_mutex):
            ...
            if self.files_completed >= self.total_files:
                # All done - create and emit DataFrame
                with QMutexLocker(self.results_mutex):
                    ...
                    if final_results:
                        ...
                        results_df = pd.DataFrame(final_results, dtype=str)
                    else:
